...
@retry(
    wait=wait_random_exponential(multiplier=1, max=60),  # max is set to 60 seconds
    stop=stop_after_attempt(3),
    retry=retry_if_exception_type(APIConnectionError) | retry_if_exception_type(APIError) | retry_if_exception_type(RateLimitError),
)
def chat_completion_code_review(messages: List[dict], file_contents: str, chat_model: str) -> ChatCompletionCodeReviewResult:
    ...
    """
    try:
        logger.debug(f"Making API call at {datetime.now()}")
        response = openai.ChatCompletion.create(
            model=chat_model,
            messages=messages,
            temperature=0.9,
        )
        logger.debug(f"API call successful at {datetime.now()}")
        assistant_reply = response.choices[0].message
        logger.debug(f"Assistant reply: {assistant_reply}")
        ...
        logger.error(f"RateLimitError at {datetime.now()}: {str(e)}")
        if hasattr(e, 'headers'):
            reset_time = e.headers.get('Retry-After', None)
            logger.error(f"Rate limit resets after: {reset_time} seconds")
            # If the header is present, we wait for that time plus a small buffer
            if reset_time:
                time_to_wait = int(reset_time) + 1  # Adding a buffer of 1 second
                logger.info(f"Waiting for {time_to_wait} seconds before retrying...")
